...
if [[ $1 == 'objdump' ]]; then
  ...
  docker run -it -v "$(pwd)/kernel.elf":/kernel --name qemu --rm ghcr.io/lowski/qemu-portux-920t:latest
elif [[ $1 == 'docker-gdb' ]]; then
  docker run -it -p 4444:4444 -v "$(pwd)/kernel.elf":/kernel --name qemu --rm ghcr.io/lowski/qemu-portux-920t:latest -nographic -M portux920t -m 64M -kernel /kernel -S -gdb tcp::4444
elif [[ $1 == 'gdb' ]]; then
